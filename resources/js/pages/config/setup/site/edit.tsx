...
const Edit = ({ id, timezones, onUpdated, onError }: EditProps): ReactNode => {
    /*** inertia js ***/
    const { data, setData, put, processing, errors, reset, clearErrors } = useForm<Required<SiteForm>>({
        entity: '',
        entity_name: '',
        code: '',
        name: '',
    ...
    const fetchData = async () => {
        const response = await axios.get(route('config.setup.site.show', { id: id }));
    ...
    const submitForm: FormEventHandler = (e: React.FormEvent<Element>) => {
        ...
        put(route('config.setup.site.update', { id: id }), {
